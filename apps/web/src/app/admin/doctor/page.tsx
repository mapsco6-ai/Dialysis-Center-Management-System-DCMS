  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-6 text-sm text-slate-500">
      ...
      <div className="mt-2 flex gap-4">
        <Link href="/admin/schedule" className="text-slate-700 hover:underline">{t("الجدول اليومي", "Daily schedule")}</Link>
        <Link href={`/admin/patients/${patientId}`} className="text-slate-700 hover:underline">{t("ملف المريض", "Patient record")}</Link>
      </div>
    </div>
  );
}
