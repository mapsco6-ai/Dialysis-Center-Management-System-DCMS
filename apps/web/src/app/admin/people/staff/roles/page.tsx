  ...
  return (
    <AdminShell user={user}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="text-xl font-semibold text-slate-800">{t("مصفوفة الأدوار والصلاحيات", "Roles & permissions matrix")}</h1>
        <Link href="/admin/people/staff" className="text-sm text-slate-500 underline">{t("← الموظفون", "← Staff")}</Link>
      </div>
      <p className="mt-1 text-sm text-slate-500">{t("عدّل الصلاحيات ثم احفظ كل دور. تسري التغييرات فوراً على الموظفين المعنيين.", "Edit permissions, then save each role. Changes apply immediately to the people holding it.")}</p>
      ...
      {canEdit && (
        <form onSubmit={create} className="mt-3 flex flex-wrap gap-2">
          ...
          <button type="submit" className="rounded-md bg-slate-800 px-3 py-1.5 text-sm font-medium text-white">{t("إضافة دور", "Add role")}</button>
        </form>
      )}
            ...
            <tr>
              ...
              {list.map((role) => (
                <th key={role.id} className="px-2 py-2 font-medium">
                  <div>{role.name}</div>
                  {canEdit && changed(role) && <button type="button" onClick={() => save(role)} className="mt-1 rounded bg-emerald-600 px-2 py-0.5 text-[10px] text-white">{t("حفظ", "Save")}</button>}
                  {canEdit && !["SUPER_ADMIN"].includes(role.name) && role.permissions.length === 0 && <button type="button" onClick={() => remove(role)} className="mt-1 block text-[10px] text-red-600 underline">{t("حذف", "Delete")}</button>}
                </th>
              ))}
            </tr>
          </thead>
...
function FragmentRows({ group, roles, draft, canEdit, onToggle }: { group: PermissionGroup; roles: Role[]; draft: Record<string, Set<string>>; canEdit: boolean; onToggle: (role: Role, key: string) => void }) {
  return (
    <>
      <tr className="bg-slate-50"><th colSpan={roles.length + 1} className="px-3 py-1 text-start text-[11px] uppercase tracking-wide text-slate-500">{group.module}</th></tr>
