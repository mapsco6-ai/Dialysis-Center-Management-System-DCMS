  ...
  const columns: TableColumn<Patient>[] = [
    {
      ...
      header: t("الرقم", "ID"),
      render: (patient) => (
        <Link href={`/admin/patients/${patient.id}`} className="text-slate-800 hover:underline">{patient.patientCode}</Link>
      ),
    },
    ...
    <AdminShell user={user}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {user.permissions.includes("patient.create") && (
          <Link href="/admin/patients/new" className="rounded-md bg-slate-800 px-4 py-2 text-sm font-medium text-white hover:bg-slate-700">
