                ...
                {canManage && (
                  <td className="px-4 py-2">
                    ...
                    {(entry.status === "OPEN" || entry.status === "ASSIGNED") && (
                      <button onClick={() => cancelTicket(entry.id)} disabled={busy} className="mt-1 block text-xs font-medium text-red-600 hover:underline disabled:opacity-50">{t("إلغاء البلاغ", "Cancel ticket")}</button>
                    )}
