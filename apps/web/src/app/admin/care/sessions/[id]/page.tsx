...
const getStatusLabels = (t: (arabic: string, english: string) => string): Record<string, string> => ({
  PRE_DIALYSIS: t("تقييم ما قبل الديلزة", "Pre-dialysis assessment"),
  SUPPLIES_READY: t("المستلزمات جاهزة", "Supplies ready"),
  WAITING_MACHINE: t("بانتظار جهاز", "Awaiting machine"),
  ...
  DISCHARGED: t("تم الخروج", "Discharged"),
    ...
    <AdminShell user={user}>
      <div className="flex items-center justify-between">
        ...
        <span className="rounded-md bg-slate-100 px-3 py-1.5 text-sm font-medium text-slate-700">
          {session ? (statusLabel[session.status] ?? session.status) : t("لم تبدأ بعد", "Not started")}
        </span>
      </div>
      ...
      {session && session.status === "PRE_DIALYSIS" && (
        <section className="mt-4 rounded-lg border border-slate-200 bg-white p-6">
          <h2 className="mb-2 text-sm font-semibold text-slate-500">{t("تقييم ما قبل الديلزة", "Pre-dialysis assessment")}</h2>
      ...
      {session && session.status === "ASSIGNED" && canStart && (
        <>
          <section className="mt-4 rounded-lg border border-slate-200 bg-white p-6">
            <VitalsSummary session={session} />
            <p className="mt-2 text-sm text-emerald-600">{t("تم تعيين الجهاز:", "Assigned machine:")} {session.machine?.machineCode}</p>
          </section>
      ...
      {session && (session.status === "IN_DIALYSIS" || session.status === "POST_DIALYSIS") && (
        <>
          ...
          <section className="mt-4 rounded-lg border border-slate-200 bg-white p-6">
            <h2 className="mb-3 text-sm font-semibold text-slate-500">{t("القراءات الدورية", "Periodic readings")}</h2>
            <ul className="space-y-1 text-sm">
              {readings.map((r) => (
                <li key={r.id} className="flex items-center justify-between border-b border-slate-50 py-1">
                  <span>
                    ...
                    {r.amendedFromId && <span className="ms-2 text-xs text-amber-600">{t("(تعديل)", "(Amended)")}</span>}
                  </span>
              ...
              {readings.length === 0 && <li className="text-slate-400">{t("لا توجد قراءات بعد", "No readings yet")}</li>}
            </ul>
            {amendingId && (
              <AmendReadingForm
                busy={busy}
          ...
          <section className="mt-4 rounded-lg border border-slate-200 bg-white p-6">
            <h2 className="mb-3 text-sm font-semibold text-slate-500">{t("أحداث الجلسة", "Session events")}</h2>
            <ul className="space-y-1 text-sm">
              {events.map((e) => (
              ...
              {events.length === 0 && <li className="text-slate-400">{t("لا توجد أحداث بعد", "No events yet")}</li>}
            </ul>
            {canEvent && (
              <EventForm
                busy={busy}
      ...
      {session && (session.status === "COMPLETED" || session.status === "DISCHARGED") && (
        <section className="mt-4 rounded-lg border border-slate-200 bg-white p-6">
          <h2 className="mb-2 text-sm font-semibold text-slate-500">{t("ملخص الجلسة", "Session summary")}</h2>
          <VitalsSummary session={session} />
...
function PreDialysisForm({ busy, onSubmit }: { busy: boolean; onSubmit: (body: unknown) => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4 max-w-lg space-y-2 rounded-lg border border-slate-200 bg-white p-4">
      <h2 className="text-sm font-semibold text-slate-500">{t("تقييم ما قبل الديلزة", "Pre-dialysis assessment")}</h2>
      <input name="weight" type="number" step="0.1" required placeholder={t("الوزن", "Weight")} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
      <input name="bp" required placeholder={t("ضغط الدم (مثال: 120/80)", "Blood pressure (e.g. 120/80)")} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
      <input name="pulse" type="number" required placeholder={t("النبض", "Pulse")} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
      <input name="temperature" type="number" step="0.1" placeholder={t("الحرارة", "Temperature")} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
      <input name="glucose" type="number" step="0.1" placeholder={t("السكر", "Blood glucose")} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
      <input name="dryWeight" type="number" step="0.1" placeholder={t("الوزن الجاف", "Dry weight")} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
      <input name="notes" placeholder={t("ملاحظات", "Notes")} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
...
function StartDialysisForm({ busy, onSubmit }: { busy: boolean; onSubmit: (body: unknown) => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4 max-w-lg space-y-2 rounded-lg border border-slate-200 bg-white p-4">
      <h2 className="text-sm font-semibold text-slate-500">{t("بدء الديلزة (START DIALYSIS)", "Start dialysis")}</h2>
      <input name="dialyzerType" required placeholder={t("نوع الدياليزر", "Dialyzer type")} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
      <input name="bloodLineType" required placeholder={t("نوع خطوط الدم", "Bloodline type")} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
      <input name="prescribedDurationMinutes" type="number" required placeholder={t("المدة الموصوفة (دقيقة)", "Prescribed duration (minutes)")} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
      <input name="requiredUF" type="number" step="0.1" required placeholder={t("UF المطلوب", "Required UF")} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
...
function ReadingForm({ busy, onSubmit }: { busy: boolean; onSubmit: (body: unknown) => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-3 flex flex-wrap items-center gap-2 rounded-md bg-slate-50 p-3">
      <input name="bp" required placeholder={t("ضغط الدم", "BP")} className="w-20 rounded-md border border-slate-300 px-2 py-1 text-xs" />
      <input name="pulse" type="number" required placeholder={t("النبض", "Pulse")} className="w-16 rounded-md border border-slate-300 px-2 py-1 text-xs" />
      <input name="arterialPressure" type="number" placeholder={t("الضغط الشرياني", "Arterial pressure")} className="w-20 rounded-md border border-slate-300 px-2 py-1 text-xs" />
      <input name="venousPressure" type="number" placeholder={t("الضغط الوريدي", "Venous pressure")} className="w-20 rounded-md border border-slate-300 px-2 py-1 text-xs" />
      <input name="tmp" type="number" placeholder="TMP" aria-label={t("الضغط عبر الغشاء", "Transmembrane pressure")} className="w-16 rounded-md border border-slate-300 px-2 py-1 text-xs" />
      <input name="bloodFlow" type="number" placeholder={t("تدفق الدم", "Blood flow")} className="w-16 rounded-md border border-slate-300 px-2 py-1 text-xs" />
      <input name="uf" type="number" step="0.1" placeholder="UF" aria-label={t("الترشيح الفائق", "Ultrafiltration")} className="w-16 rounded-md border border-slate-300 px-2 py-1 text-xs" />
...
function AmendReadingForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-3 flex flex-wrap items-center gap-2 rounded-md bg-amber-50 p-3">
      <input name="bp" required placeholder={t("BP الصحيح", "Correct blood pressure")} className="w-20 rounded-md border border-slate-300 px-2 py-1 text-xs" />
      <input name="pulse" type="number" required placeholder={t("Pulse الصحيح", "Correct pulse")} className="w-20 rounded-md border border-slate-300 px-2 py-1 text-xs" />
...
function EventForm({ busy, onSubmit }: { busy: boolean; onSubmit: (body: unknown) => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-3 flex flex-wrap items-center gap-2 rounded-md bg-slate-50 p-3">
      ...
      <input name="note" placeholder={t("ملاحظة", "Note")} className="w-48 rounded-md border border-slate-300 px-2 py-1 text-xs" />
      <button type="submit" disabled={busy} className="rounded bg-slate-800 px-3 py-1 text-xs text-white disabled:opacity-50">
...
function ReassignMachineForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4 max-w-lg space-y-2 rounded-lg border border-red-200 bg-red-50 p-4">
      <h2 className="text-sm font-semibold text-red-700">{t("إعادة تعيين جهاز (عطل أثناء الجلسة)", "Reassign machine (fault during session)")}</h2>
      ...
      <input name="reason" required placeholder={t("سبب العطل", "Reason for fault")} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
...
function EndDialysisForm({ busy, onSubmit }: { busy: boolean; onSubmit: (body: unknown) => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4 max-w-lg space-y-2 rounded-lg border border-slate-200 bg-white p-4">
      <h2 className="text-sm font-semibold text-slate-500">{t("إنهاء الديلزة (END DIALYSIS)", "End dialysis")}</h2>
      <input name="postWeight" type="number" step="0.1" required placeholder={t("الوزن بعد الجلسة", "Post-dialysis weight")} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
      <input name="postBP" required placeholder={t("ضغط الدم بعد الجلسة", "Post-dialysis blood pressure")} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
      <input name="postPulse" type="number" required placeholder={t("النبض بعد الجلسة", "Post-dialysis pulse")} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
      <input name="actualUF" type="number" step="0.1" required placeholder={t("UF الفعلي", "Actual UF")} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
      <input name="complications" placeholder={t("مضاعفات (إن وجدت)", "Complications (if any)")} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
      <input name="finalNote" placeholder={t("ملاحظة ختامية", "Final note")} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-sm" />
      <button type="submit" disabled={busy} className="rounded-md bg-slate-800 px-3 py-1.5 text-xs font-medium text-white hover:bg-slate-700 disabled:opacity-50">
    ...
    <div className="mt-4">
      {!open ? (
        ...
        <form onSubmit={handleVerify} className="flex flex-wrap items-center gap-2 rounded-md bg-slate-50 p-3">
          <input value={username} onChange={(e) => setUsername(e.target.value)} placeholder={t("اسم المستخدم", "Username")} className="w-32 rounded-md border border-slate-300 px-2 py-1 text-xs" />
          <input value={pin} onChange={(e) => setPin(e.target.value)} placeholder="PIN" aria-label={t("رمز التعريف الشخصي", "Personal identification number")} inputMode="numeric" className="w-20 rounded-md border border-slate-300 px-2 py-1 text-xs" />
          <button type="submit" disabled={busy} className="rounded bg-slate-800 px-3 py-1 text-xs text-white disabled:opacity-50">
            {t("تحقق", "Verify")}
          </button>
